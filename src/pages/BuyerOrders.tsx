...
import {
  ...
  Clock, Search, Filter, X, Eye, Hash,
  Copy, Globe, Loader2, Inbox, Map as MapIcon
} from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-black tracking-tight mb-1 flex items-center">
          <ShoppingCart className="mr-3 text-emerald-500" /> My Matches
        </h1>
        <p className="text-slate-500 dark:text-slate-400 font-medium">
          Track waste you've matched with and completed purchases.
        </p>
      </div>
        ...
        <div className="flex-1 flex gap-3">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              placeholder="Search waste type, seller..."
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800">
          ...
          <h3 className="text-xl font-bold mb-2 text-slate-700 dark:text-slate-300">No {tab === 'active' ? 'Active' : 'Past'} Matches</h3>
          <p className="text-slate-500 text-sm max-w-sm text-center">
            {tab === 'active'
              ? 'Head to AI Matcher or Browse Waste to find circular economy partners.'
              : 'Completed and rejected matches will appear here.'}
          </p>
        </div>
      ) : (
        <div className="grid gap-5">
          {filtered.map(match => {
            ...
            const sellerName = match.listing?.seller?.companyName || match.listing?.seller?.name || 'Seller';
            const sellerLocation = match.listing?.seller?.location || match.listing?.location || 'N/A';
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 flex items-center font-medium">
                          ...
                          Seller: {sellerName} • {sellerLocation}
                        </p>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2 text-xs font-bold">
                      <div className="flex items-center text-slate-600 bg-slate-100 dark:bg-slate-800 px-2.5 py-1.5 rounded-lg">
                        <ShoppingCart size={12} className="mr-1.5 text-slate-400" />
                    ...
                    <div className="mt-4 flex gap-3">
                      <button
                        ...
                        className="inline-flex items-center gap-2 bg-slate-100 dark:bg-slate-800 hover:bg-sky-50 dark:hover:bg-sky-500/10 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-xl text-xs font-bold tracking-wider transition-all active:scale-95 border border-slate-200 dark:border-slate-700"
                      >
